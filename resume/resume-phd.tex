...
Vision models that hold up outside the benchmark. Interpretability a domain expert can act on, representations that transfer to scientific imaging, and control over generative models that cannot be retrained. Building ML for a regulated clinical platform keeps the work tied to real decisions.
...
\begin{itemize}
    \item Built a model that flags Alzheimer's residents at risk of decline from their daily activity and caregiver logs, so staff can intervene earlier; 0.81 AUC on held-out data.
    \item Built an NLP pipeline that turns free-text clinical notes into structured records of symptoms, medications, and care events (0.90 F1), and ran both models in production on de-identified data under HIPAA.
\end{itemize}
...
\begin{itemize}
    ...
    \item Built the real-time feature pipeline that feeds them (SignalR, SQL Server change tracking) and delivered predictions to users through Blazor dashboards.
\end{itemize}
